...
public class S13_matchingStrings {
    ...
    public static boolean chkMatching(char[] str1, char[] patstr, int n, int m, boolean[][] lookup) {
        ...
        if (!lookup[m][n]) {
            if (patstr[m] == '*') {
                // If pattern contains '*', check for matching by moving in both directions in
                // pattern.
                lookup[m][n] = chkMatching(str1, patstr, n - 1, m, lookup) ||
                        chkMatching(str1, patstr, n, m - 1, lookup);
            } else {
                // If characters at current positions match or are '?' (wildcard), move to the
                // previous positions.
